import { IonPage, IonList,IonItem,IonLabel,IonContent,IonButton,IonCheckbox } from '@ionic/react'
...
const LevelAddStudent = () => {
    ...
    const [level, setLevel] = React.useState({level_name: "", grade: "5", name:"A"})
    ...
    const addStudents = (e) => {
        ...
        callApi({url: `/levels/${level_id}/students`, method: "post", params: {student_ids: students.join(",")}}).then((data) => {
            if(data) {
                showMessage(`${students.length} ${labels.student}(s) added to this ${labels.level} Successfully`, "success")
    ...
    return (
        <IonPage>
            <Title name={ `Add ${labels.student} to ${level.level_name} ${labels.level}` } back={`/shelters/${shelter_id}/projects/${project_id}/levels/${level_id}`} />
                        ...
                        { (students.map((student, index) => {
                            return ( 
                                <IonItem key={index} className="striped">
                                    <IonCheckbox value={ student.id } checked={ selected[student.id] } onIonChange={ selectStudent } slot="start"></IonCheckbox>
                                    <IonLabel>{student.name}</IonLabel>
                                </IonItem>
                            )
                        })) }

                        <IonItem><IonButton onClick={ addStudents }>Add Selected {labels.student} to {level.level_name} {labels.level}</IonButton></IonItem>
                    </form>
                </IonList>
            </IonContent>
        </IonPage>
