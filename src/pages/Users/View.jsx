      ...
      <IonContent>
        ...
        {segment === 'details' ? (
          <IonGrid>
            ...
            {user.mad_email ? (
              <LabeledRow label="MAD EMail" value={user.mad_email} />
            ) : null}
            <LabeledRow
              label="Groups"
              value={user.groups
            ...
            <LabeledRow
              label="Sex"
              value={{ m: 'Male', f: 'Female', o: 'Other' }[user.sex]}
            />
            <LabeledRow
              label="Volunteer Since"
              value={moment(user.joined_on).format('YYYY, MMMM Do')}
            />
          </IonGrid>
        ) : null}
...
const LabeledRow = ({ label, value }) => {
  return (
    <IonRow>
      <IonCol className="label" size-xs="4" size-md="2" size-lg="1">
        {label}
      </IonCol>
      <IonCol>{value}</IonCol>
    </IonRow>
