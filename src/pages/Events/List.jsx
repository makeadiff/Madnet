...
const EventList = ({ segment }) => {
  ...
  useEffect(() => {
    async function fetchEventList() {
      ...
      if (segment === 'invitations') {
        events_data = await callApi({
          url:
            `events?invited_user_id=${user.id}&from_date=today&to_date=` +
        ...
        events_data = await callApi({
          url:
            `events?city_id=${user.city_id}&from_date=today&to_date=` +
            moment().add(7, 'd').format('YYYY-MM-DD'),
          method: 'get',
          cache: true,
    ...
    if (segment === 'invitations') {
      if (
        cache[`user_${user.id}_events`] === undefined ||
        !cache[`user_${user.id}_events`]
      ) {
        fetchEventList()
      } else {
      ...
      if (
        cache[`city_${user.city_id}_events`] === undefined ||
        !cache[`city_${user.city_id}_events`]
      ) {
        fetchEventList()
      } else {
  ...
  return (
    <IonList>
      ...
      {events.length === 0 ? (
        <IonItem>
          <IonLabel className="error">No Events found.</IonLabel>
        </IonItem>
      ) : null}
    </IonList>
  )
