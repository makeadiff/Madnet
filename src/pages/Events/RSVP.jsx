  ...
  React.useEffect(() => {
    async function fetchEvent() {
      ...
      if (event_data.event !== undefined) {
        ...
        const rsvp = await api.rest(
          `events/${eventId}/users/${user.id}`,
          'get',
          {},
          user.jwt_token
        )
        ...
        <IonGrid>
          <IonCol size-md="6">
            <IonCard className="light">
              <IonCardHeader>
                <IonCardTitle>RSVP for {event.name}</IonCardTitle>
              </IonCardHeader>
              <IonCardContent>
                <IonRow>
                  {event.starts_on ? (
                    <IonCol size-xs="3" size-md="1">
                      <div className="date-number">
                        {moment(event.starts_on).format('D')}
                      </div>
                  ...
                  <IonCol size-xs="9" size-md="11">
                    <IonRow className="actions">
                      <IonCol
                        className={`ion-text-center going ${
                          rsvp === 'going' ? 'selected' : null
                        }`}
                      ...
                      <IonCol
                        className={`ion-text-center cant_go ${
                          rsvp === 'cant_go' ? 'selected' : null
                        }`}
                        onClick={() => {
                          saveRsvp('cant_go')
                        }}
                      ...
                      <IonCol
                        className={`ion-text-center maybe ${
                          rsvp === 'maybe' ? 'selected' : null
                        }`}
                ...
                <IonRow>
                  <IonCol>
                    ...
                    <IonLabel>
                      Event Data-Time:{' '}
                      {moment(event.starts_on).format('MMM Do, hh:mm A')}
                    </IonLabel>
                  </IonCol>
                </IonRow>
                {event.place ? (
