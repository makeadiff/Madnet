...
function Login() {
  ...
  const loginUser = (user_data, method) => {
    ...
    if (user) {
      ...
      if (method === 'google') {
        showMessage(
          "Can't find any registered user associated with the given email",
          'error'
        )
      } else {
  ...
  const authHandler = () => {
    ...
    api
      .rest(
        `users/login?identifier=${userEmail}&password=${userPassword}`,
        'get'
      )
      ...
      <IonContent className="ion-justify-content-center dark">
        <IonCard className="dark loginCard">
          <IonItem className="logoHolder">
            <IonImg
              src={
                process.env.PUBLIC_URL + '/assets/icon/madnet/madnet-100.png'
              }
            ></IonImg>
          </IonItem>
          <IonCardHeader>
