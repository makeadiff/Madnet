...
const StarRating = ({ min, max, value, onChange, style }) => {
  ...
  React.useEffect(() => {
    // This should ideally go in the React.useState(value) - but that's not working for some reason.
    setRating(value)
  }, [value])

  React.useEffect(() => {
    if (onChange !== undefined) {
      // :TODO: Make sure this is a function.
      onChange(rating)
    }
  }, [rating])
...
const Star = ({ value, filled, style, onClick }) => {
  ...
  return (
    <span onClick={onClick}>
      <IonIcon value={value} slot="start" icon={icon} style={style} />
    </span>
  )
}
