describe('MADNet App Test', function () {
  ...
  it('should show error if no email or password is given', async () => {
    ...
    expect(
      await element(by.css('ion-input[name="email"] input')).getAttribute(
        'validationMessage'
      )
  ...
  it('should login with valid creds', async () => {
    await element(by.css('ion-input[name="email"] input'))
      .clear()
      .sendKeys('picard.simulation@makeadiff.in')
    await element(by.css('ion-input[name="password"] input'))
      .clear()
      .sendKeys('mad4ever')
    expect(
      await element(by.css('ion-input[name="email"] input')).getAttribute(
